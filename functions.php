...
function op_view_user($con,$user_id,$operator_username){
	...
	return $a=mysqli_query($con,"SELECT jobs.*,user.id,user.name,user.email,user.phone_no,user.date,user.country,user.alt_phone FROM jobs,user WHERE user.id=jobs.id AND jobs.operator='".$operator_username."' AND jobs.id='".$user_id."'");
}
//user jobs
